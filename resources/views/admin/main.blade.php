    ...
    */
            ...
            $.ui.fancytree.registerExtension({
                ...
                options: {
                    ...
                    dropMarkerOffsetX: -
                    24, // absolute position offset for .fancytree-drop-marker relatively to ..fancytree-title (icon/img near a node accepting drop)
                    dropMarkerInsertOffsetX: -
                    16, // additional offset for drop-marker with hitMode = "before"/"after"
                    // Events (drag support)
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    <li class="nav-item">
                        <a class="nav-link active" aria-current="page" href="{{ route('admin.dashboard') }}">Главная</a>
                    </li>
                    <li class="nav-item">
    ...
    <script>
        $(function(){
            $("#tree").fancytree({
                ...
                renderNode: function(event, data) {
                    ...
                    if (!$nodeSpan.data("rendered")) {
                        $nodeSpan.append('<span class="fancytree-buttons">' +
                            '<button class="btn btn-warning" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;">Изменить</button>' +
                            '<button class="btn btn-danger" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;">Удалить</button>' +
                            '</span>');
            ...
            $(document).on("click", ".editBtn", function() {
                ...
                $("#editCategoryId").val(node.key);
                $("#editCategoryName").val(node.title);
                $("#editModal").show();
            });
            ...
            $(document).on("click", ".deleteBtn", function() {
                ...
                if (confirm("Вы уверены, что хотите удалить эту категорию?")) {
                    // AJAX-запрос для удаления категории
                    $.ajax({
                        url: "/dashboard/categories/" + node.key,
                        type: "DELETE",
            ...
            $("#editForm").on("submit", function(e) {
                ...
                // AJAX-запрос для обновления категории
                $.ajax({
                    ...
                    success: function(result) {
                        var node = $("#tree").fancytree("getTree").getNodeByKey(categoryId);
